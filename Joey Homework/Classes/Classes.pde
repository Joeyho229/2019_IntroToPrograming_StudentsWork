...
void setup() {    // Her bliver tingene kun "tegnet" 1 gang.
  ...
  Suns[0] = new Sun(xPosition[2], yPosition[2], SunRadius[0], sc, rc, Rays[0]);
  Suns[1] = new Sun(xPosition[3], yPosition[3], SunRadius[1], sc, rc, Rays[1]);
  ...
  for (int i = 0; i < num; i++) {    // Et for loop hvor "i" får nogle værdier, den er lig 0 men også mindre end variablen num og den addere med sig selv hver gang "i++"
    SunTrail[i] = new Sun(xPosition[2], yPosition[2], SunRadius[0], sc, rc, Rays[0]);    // Fortæller at hver eneste gang SunTrail[i] bliver fremkaldt, så har de disse oplyste værdier.
  }
}
